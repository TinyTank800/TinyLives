...
public class SetlifeHandler {

    public static void Setlife(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args){
        if(args.length == 1){
            sender.sendMessage(Parser.translateColor("&cInvalid usage of /tinylives setlife! EX: /tinylives setlife (playername) <amount>."));
            return;
        }
        ...
        if(args.length >= 3){
            ...
            if(player != null){
                PlayerUtils.SetLives(player, amount);
                return;
            } else {
                //Get offline player through yaml
            }
