    ...
    public static void Lives(@NotNull Player sender, @NotNull Command command, @NotNull String label, String[] args){
        ...
        if(args.length >= 1){
            if(sender.hasPermission("tinylives.lives.others")){
                Player player = Tinylives.getInstance().getServer().getPlayer(args[0]);
                if(player == null){
                    return;
                }

                lives = Configs.Get("playerdata.yml").getInt("players." + player.getUniqueId() + ".lives");
                sender.sendMessage(Parser.format(sender,Configs.Get("config.yml").getString("prefix") + "&a " + player.getName() + " currently has &f" + lives + " live/s&a."));
            }
        } else {
            lives = Configs.Get("playerdata.yml").getInt("players." + sender.getUniqueId() + ".lives");
            sender.sendMessage(Parser.format(sender,Configs.Get("config.yml").getString("prefix") + "&a you currently have &f" + lives + " lives&a."));
        }
    }
}
