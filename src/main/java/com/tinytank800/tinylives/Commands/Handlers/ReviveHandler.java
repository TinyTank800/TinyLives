...
public class ReviveHandler {
    ...
    public static void ReviveConsole(@NotNull ConsoleCommandSender sender, @NotNull Command command, @NotNull String label, String[] args){
        sender.sendMessage(Parser.translateColor("&cOnly players can use this command."));
    }

    public static void RevivePlayer(@NotNull Player sender, @NotNull Command command, @NotNull String label, String[] args){
        if(args.length == 1){
            if(Configs.Get("playerdata.yml").getInt("players." + sender.getUniqueId() + ".lives") <= Configs.Get("config.yml").getInt("death-settings.revive.cost")){
                sender.sendMessage(Parser.translateColor("&cYou cannot revive this player as you do not have enough lives to give."));
                return;
